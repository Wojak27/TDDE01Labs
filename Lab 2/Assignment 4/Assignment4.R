...
par(mfcol = c(1, 2))
plot(1:length(S[,1]), S[,1], type = "l",xlab = "S1", ylab = "")
plot(1:length(S[,2]), S[,2], type = "l", xlab = "S2", ylab = "")

#mixing components
X <-as.matrix(S) %*% A
par(mfcol = c(1, 2))
plot(1:length(X[,1]), X[,1], type = "l",xlab = "X1", ylab = "")
plot(1:length(X[,2]), X[,2], type = "l", xlab = "X2", ylab = "")
...
par(mfcol = c(1, 2))
plot(1:length(a$S[,1]), a$S[,1], type = "l", xlab = "S'1", ylab = "")
plot(1:length(a$S[,2]), a$S[,2], type = "l", xlab = "S'2", ylab = "")

W.prim = a$K*a$W

plot(1:length(W.prim), W.prim, type = "l", xlab = "S'1", ylab = "")
plot(1:length(W.prim), W.prim, type = "l", xlab = "S'2", ylab = "")
