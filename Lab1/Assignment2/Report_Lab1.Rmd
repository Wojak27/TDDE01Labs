...
ggplot(df, aes(theta, maxlik, colour = cat))+
  geom_point()
````
For the assignment 1.4 we have a Bayesian model with function $\mathrm{p}(x \mid \theta)=\theta e^{-\theta x}$ and a prior function $\mathrm{p}(\theta)=\lambda  e^{-\theta \lambda}$ where $\lambda = 10$ and we want to show the dependence of function $l(\theta)=\log(\mathrm{p}(x \mid \theta)\mathrm{p}(\theta))$ on $\theta$ and plot the function with the previous ones. In the graph below we can se how similar the bayesian model is to the model that uses the entire dataset. The baysian model decreases a bit faster than the first model as well as it has a slightly different maximum value. The maximum likelihood for the functions are following:
...
mse <- function(linProtMois) 
    mean(linProtMois$residuals^2)
